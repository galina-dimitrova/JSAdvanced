function initializeTable() {
    ...
    function fixRowLinks() {
        $('#countriesTable a').css('display', 'inline'); // show all links
        let tableRows = $('#countriesTable tr'); //hide Up link in first data row
        $(tableRows[2]).find("a:contains('Up')")
        .css('display', 'none');

        $(tableRows[tableRows.length-1]).find("a:contains('Down')") //hide Down in last table row
        .css('display', 'none');
    }
}